...
const handlePriceChange = (value) => {
  ...
  // -------------------- FIXED: Area Validation --------------------
  const handleAreaChange = (areaType, value) => {
    if (value && parseFloat(value) > 0) {
      if (areaType === 'builtupArea' && !form.getFieldValue('carpetArea')) {
        Toast.show({
          content: "Please fill carpet area first",
          duration: 3000
        });
        setActiveAreaType('carpetArea');
        setShowAreaInputs(true);
        
        setTimeout(() => {
          const carpetField = document.querySelector('[name="carpetArea"]');
          if (carpetField) {
            carpetField.scrollIntoView({ behavior: 'smooth', block: 'center' });
            carpetField.focus();
          }
        }, 100);
      }
  ...
  // -------------------- COMPLETELY FIXED: Mobile Media Handler Functions --------------------
  ...
  switch (fieldName) {
    ...
    case 'carpetArea':
      if (!value || parseFloat(value) <= 0) {
        ...
        errors.carpetArea = 'Carpet area seems too large';
      }
      break;
...
const validateFormData = () => {
  ...
  // Area validation - accept any area type
  const carpetArea = values.carpetArea || "";
  const builtupArea = values.builtupArea || "";
  const superBuiltupArea = values.superBuiltupArea || "";
  
  const hasAnyArea = 
    (carpetArea && !isNaN(parseFloat(carpetArea)) && parseFloat(carpetArea) > 0) ||
    (builtupArea && !isNaN(parseFloat(builtupArea)) && parseFloat(builtupArea) > 0) ||
    (superBuiltupArea && !isNaN(parseFloat(superBuiltupArea)) && parseFloat(superBuiltupArea) > 0);

  if (!hasAnyArea) {
    errors.area = 'At least one area type is required';
    missing.push('Area');
  }
...
const showValidationErrors = (errors, missing) => {
  ...
  try {
    ...
    // ⚡ ADD IMAGES DIRECTLY (NO LOGGING - faster!)
    images.forEach((img) => {
      if (img.file) formData.append("images", img.file);
    });
    
    // ⚡ ADD VIDEOS (no logging)
    videos.forEach((vid) => {
      if (vid.file && vid.file.size <= 50 * 1024 * 1024) {
    ...
    if (result.success) {
      Toast.show(editMode ? "Property updated!" : "Property posted!", { duration: 1000 });
      onClose?.();
      window.dispatchEvent(new Event("propertyAdded"));
    } else {
      Toast.show(result.error || "Failed to post property.");
    }
  } catch (err) {
  ...
  return (
    ...
    >
      <div className="post-property-popup" style={{ height: '100%', overflow: 'hidden' }}>
        ...
        <div className="popup-content" style={{ height: 'calc(100% - 80px)', overflow: 'auto' }}>
          ...
          >
              ...
              <Space direction="vertical" block style={{ width: '100%' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: '#666', marginTop: '4px' }}>
                    ...
                    Actual Value: {actualPriceValue ? `₹${parseInt(actualPriceValue).toLocaleString('en-IN')}` : '₹0'}
                  </div>
                </div>
                ...
                <div style={{ marginTop: '16px' }}>
                  ...
                  {!showAreaInputs && (
                    <Form.Item 
                      name={activeAreaType}
                      rules={[{ required: activeAreaType === 'carpetArea', message: `Please enter ${AREA_TYPES.find(a => a.value === activeAreaType)?.label}` }]}
                    ...
                    <Space direction="vertical" block style={{ width: '100%' }}>
                      {AREA_TYPES.map((areaType) => (
                        <Form.Item 
                          ...
                          rules={[{ required: areaType.value === 'carpetArea', message: `Please enter ${areaType.label}` }]}
            ...
            {/* FIXED: Media Upload - Mobile Compatible */}
            <Card title="Media" style={{ marginBottom: '16px', borderRadius: '12px' }}>
              <Space direction="vertical" block style={{ width: '100%' }}>
                {/* Images Section - FIXED for Mobile */}
                <div style={{ marginBottom: '16px' }}>
                  <div style={{ 
                    display: 'flex', 
                    justifyContent: 'space-between', 
                ...
                {/* Videos Section - FIXED for Mobile */}
                <div>
                  <div style={{ 
                    display: 'flex', 
                    justifyContent: 'space-between', 
      ...
      {/* Image Action Sheet */}
      {/* Image Action Sheet - Only one option */}
<ActionSheet
...
{/* Video Action Sheet - Only one option */}
<ActionSheet
